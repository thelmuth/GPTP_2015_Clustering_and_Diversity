...
%\begin{figure}[!t]
%	\centering
%	\fbox{\parbox{8.2cm}{To select a parent for use in a genetic operation:
%			\begin{enumerate}
%				\item {\ttfamily Initialize}:
%				\begin{enumerate}
%					\item Set {\ttfamily candidates} to be the entire population.
%					\item Set {\ttfamily cases} to be a list of all of the test cases in random order.
%				\end{enumerate}
%				\item {\ttfamily Loop}:
%				\begin{enumerate}
%					\item Set {\ttfamily candidates} to be the subset of the current {\ttfamily candidates} that have exactly the best performance of any individual currently in {\ttfamily candidates} for the first case in {\ttfamily cases}.
