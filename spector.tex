...
\marginpar{We should tell what linkage we're using to combine clusters (I think it's average, but we should check).}
...
\marginpar{This is incorrect. Firstly, I believe there are 200 test cases for this problem, but I should double check. Secondly, half the test cases use levenshtein distance between correct output and program's output. So, the last character will be part of that. The other half the test cases look if the last printed char is the correct checksum char, but don't require "Check sum is" printed first.}
...
traction on the problem.%\marginpar{
